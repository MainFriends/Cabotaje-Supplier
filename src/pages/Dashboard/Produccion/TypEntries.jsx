...
import AddTypEntries from '../../../components/TypeEntries/AddTypEntries'
import EditTypEntries from '../../../components/TypeEntries/EditTypEntries'
    ...
    const TypEntries = () => {  
        ...
        const columns = [
            ...
            },
            {
                ...
                button: true,
                cell: row => <>
                    <button className={'btn btn-sm btn-warning mr-1 ' + (!permissions.UPD ? ' disabled' : null)} onClick={() => {setRowCOD(row.COD_TYPE)}} data-toggle="modal" data-target='#editFormTypEntries'><i className="fa-solid fa-pen-to-square"></i></button>
        ...
        const handleDelete = (cod) => {
            axios.delete(`/typeEntries/${cod}`, token())
        ...
        return (
                ...
                <div className="card shadow rounded">
                    <div className="card shadow rounded"></div>
                    ...
                    <div className="card-body">
                        <div className="row mt-2 ml-1">
                            <div className="col">
                                <button className={'btn btn-sm btn-primary ' + (!permissions.INS ? 'disabled' : null)} data-toggle="modal" data-target='#addTypEntries'><i className="fas fa-plus mr-2"></i>Agregar</button>
                            </div>
                        </div>
                        <DataTable
                            columns={columns}
                        ...
                        <Modal 
                            idModal='addTypEntries'
                            title='Agregar tipo de entrada'
                            messageError={messageError}
                            content={<AddTypEntries setSendRequest={setSendRequest} setMessageError={setMessageError}/>}
                        />
    
                        <Modal 
                            idModal='editFormTypEntries'
                            title='Actualizar tipo de entrada'
                            messageError={messageError}
                            content={<EditTypEntries rowCOD ={rowCOD} setSendRequest={setSendRequest} setMessageError={setMessageError}/>}
                        />
                    </div>  
             </div>                              
    )    
