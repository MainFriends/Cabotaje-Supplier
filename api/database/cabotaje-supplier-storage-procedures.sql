...
CREATE PROCEDURE SP_LOGIN(IN
    ...
    IF @USER >= 1 THEN
		SELECT USER.COD_USER, COD_ROLE, FIRST_NAME, LAST_NAME, USER_PASSWORD, DAT_EXP, COD_STATUS, NUM_ATTEMPS, USER_NEW, REQUIRED_QUESTIONS FROM USER, LOGIN
...
BEGIN
	IF _COD_INVOICE = 0 THEN
		SELECT COD_INVOICE, CONCAT(CLIENT.FIRST_NAME, ' ', CLIENT.LAST_NAME) CLIENT, RTN,
...
-- INSERT
  ...
  -- INSERTAR DATOS A LOGIN
  	INSERT INTO LOGIN(COD_USER, COD_ROLE , USER_EMAIL, USER_PASSWORD, DAT_CREATE, DAT_EXP, USER_NEW, REQUIRED_QUESTIONS) 
    VALUES(@COD_USER, _COD_ROLE, _USER_EMAIL, _USER_PASSWORD, NOW(), DATE_ADD(NOW(), INTERVAL 1 DAY), 'true', 'false'); 
...
DELIMITER //
CREATE PROCEDURE SP_SEL_SECURITY_USER_QUESTION(IN _COD_USER BIGINT
)BEGIN
	SELECT SECURITY_ANSWERS.COD_QUESTION, QUESTION, RESPONSE FROM SECURITY_ANSWERS, SECURITY_QUESTIONS
    WHERE SECURITY_ANSWERS.COD_QUESTION = SECURITY_QUESTIONS.COD_QUESTION
    AND COD_USER = _COD_USER;
END;
//
...
IF EXISTS (SELECT IDENTITY FROM CLIENT WHERE IDENTITY = _IDENTITY) OR (SELECT RTN FROM CLIENT WHERE RTN = _RTN) THEN
    ...
    INSERT INTO CLIENT( IDENTITY, FIRST_NAME, MIDDLE_NAME, LAST_NAME, SECOND_LAST_NAME, ADDRESS, NUM_PHONE_ONE,NUM_PHONE_TWO,RTN)
...
CREATE PROCEDURE SP_SEL_ROLES(
	...
	SELECT COD_ROLE, NAM_ROLE, DES_ROLE, NAM_STATUS, ROLE.COD_STATUS FROM ROLE, STATUS
    WHERE ROLE.COD_STATUS = STATUS.COD_STATUS;
END
//
...
DELIMITER //
CREATE PROCEDURE SP_UPD_ROLE(IN _COD_ROLE BIGINT,
								_COD_STATUS BIGINT
)BEGIN
	UPDATE ROLE
    SET COD_STATUS = _COD_STATUS
...
DELIMITER //
CREATE PROCEDURE SP_SEL_ROLES_USER(
)BEGIN	
	SELECT COD_ROLE, NAM_ROLE, DES_ROLE, NAM_STATUS, ROLE.COD_STATUS FROM ROLE, STATUS
    WHERE ROLE.COD_STATUS = STATUS.COD_STATUS
    AND ROLE.COD_STATUS != 2;
END
//

DELIMITER //
CREATE PROCEDURE SP_SEL_SECURITY_QUESTIONS(
)BEGIN
...
DELIMITER //
CREATE PROCEDURE SP_INS_SECURITY_ANSWERS(IN _COD_USER BIGINT,
											_COD_QUESTION_1 BIGINT,
                                            _RESPONSE_1 TEXT,
                                            _COD_QUESTION_2 BIGINT,
                                            _RESPONSE_2 TEXT,
                                            _COD_QUESTION_3 BIGINT,
                                            _RESPONSE_3 TEXT
)BEGIN
	INSERT INTO SECURITY_ANSWERS(RESPONSE, COD_QUESTION, COD_USER)
    VALUES(_RESPONSE_1, _COD_QUESTION_1, _COD_USER);
	INSERT INTO SECURITY_ANSWERS(RESPONSE, COD_QUESTION, COD_USER)
    VALUES(_RESPONSE_2, _COD_QUESTION_2, _COD_USER);
	INSERT INTO SECURITY_ANSWERS(RESPONSE, COD_QUESTION, COD_USER)
    VALUES(_RESPONSE_3, _COD_QUESTION_3, _COD_USER);
...
DELIMITER //
CREATE PROCEDURE SP_SEL_SECURITY_LOGIN(IN _COD_USER BIGINT
)BEGIN
	SELECT * FROM LOGIN WHERE COD_USER = _COD_USER;
END;
//

DELIMITER //
CREATE PROCEDURE SP_COMPARE_SECURITY_QUESTION(IN _COD_USER BIGINT,
												_COD_QUESTION_1 BIGINT,
												_RESPONSE_1 TEXT,
												_COD_QUESTION_2 BIGINT,
												_RESPONSE_2 TEXT,
	...
	SET @RESPONSE_1 = (SELECT RESPONSE FROM SECURITY_ANSWERS WHERE COD_USER = _COD_USER AND COD_QUESTION = _COD_QUESTION_1);
	SET @RESPONSE_2 = (SELECT RESPONSE FROM SECURITY_ANSWERS WHERE COD_USER = _COD_USER AND COD_QUESTION = _COD_QUESTION_2);
	SET @RESPONSE_3 = (SELECT RESPONSE FROM SECURITY_ANSWERS WHERE COD_USER = _COD_USER AND COD_QUESTION = _COD_QUESTION_3);
    
    IF @RESPONSE_1 != _RESPONSE_1 OR @RESPONSE_2 != _RESPONSE_2 OR @RESPONSE_3 != _RESPONSE_3 THEN
    		SIGNAL SQLSTATE '45000'
			SET MESSAGE_TEXT = 'Alguna de tus respuestas es incorrecta.';
    END IF;
    
    UPDATE LOGIN
    SET REQUIRED_QUESTIONS = 'false'
    WHERE COD_USER = _COD_USER;
END;
//

DELIMITER //
CREATE PROCEDURE SP_SEL_ISV(
					IN _COD_TAX BIGINT
)BEGIN
  IF _COD_TAX = 0 THEN
   SELECT COD_TAX, TAX, TAXES.COD_STATUS, NAM_STATUS FROM TAXES, STATUS
