...
BEGIN
        -- UPDATE EN LA BASE DE DATOS
           UPDATE PAY_FORM
           SET HOURS_WORKED = _HOURS_WORKED, AMO_GROSS = _AMO_GROSS, BONUS = _BONUS, TOT_DEDUCTIONS = _TOT_DEDUCTIONS, NET_SALARY = _NET_SALARY, DAT_PAYMENT = _DAT_PAYMENT
...
CREATE PROCEDURE SP_SEL_PAY_FORM(
   ...
   IF _COD_PAY_FORM = 0 THEN
	  SELECT COD_PAY_FORM, FIRST_NAME, LAST_NAME, HOURS_WORKED, AMO_GROSS, BONUS, TOT_DEDUCTIONS, NET_SALARY, DAT_PAYMENT
      FROM PAY_FORM, USER 
...
CREATE PROCEDURE SP_DEL_PAY_FORM(
 ...
 CREATE PROCEDURE SP_UPD_ACCOUNTS_TO_PAY(
    ...
    -- UPDATE EN LA BASE DE DATOS
    UPDATE ACCOUNTS_TO_PAY
    SET DESCRIPTION = _DESCRIPTION, TOT_BALANCE = _TOT_BALANCE, DATE_LIMIT = _DATE_LIMIT
    WHERE COD_ACC_PAY = _COD_ACC_PAY;
  END
 //
 ...
 CREATE PROCEDURE SP_SEL_FEES_TO_PAY(
      ...
      WHERE SUPPLIER.COD_SUPPLIER = ORDERS.COD_SUPPLIER
      AND TYPE_TO_PAY.COD_TYP_PAY = PURCHASE_INVOICE.COD_TYP_PAY
      AND ORDERS.COD_ORDER = PURCHASE_INVOICE.COD_ORDER
      AND PURCHASE_INVOICE.COD_INVOICE = ACCOUNTS_TO_PAY.COD_INVOICE
