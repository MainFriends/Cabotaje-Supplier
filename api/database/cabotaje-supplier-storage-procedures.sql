...
CREATE PROCEDURE SP_DEL_PAY_FORM(
 ...
 CREATE PROCEDURE SP_SEL_ACCOUNTS_TO_PAY(
    ...
    IF _COD_ACC_PAY = 0 THEN
       SELECT COD_ACC_PAY, NAM_SUPPLIER, NAM_CONTACT, LAST_NAM_CONTACT, NUM_PHONE_ONE, DESCRIPTION, TOT_BALANCE, DATE_LIMIT
       FROM ACCOUNTS_TO_PAY, SUPPLIER, PURCHASE_INVOICE, ORDERS
       WHERE SUPPLIER.COD_SUPPLIER = ORDERS.COD_SUPPLIER
       AND ORDERS.COD_ORDER = PURCHASE_INVOICE.COD_ORDER
 ...
 CREATE PROCEDURE SP_SEL_FEES_TO_PAY(
      ...
      SELECT COD_FEES, NAM_SUPPLIER, NAM_CONTACT, LAST_NAM_CONTACT, AMOUNT, DATE_PAY, NAM_TYPE_PAY
      FROM FEES_TO_PAY, SUPPLIER, ACCOUNTS_TO_PAY, PURCHASE_INVOICE, TYPE_TO_PAY, ORDERS
      WHERE SUPPLIER.COD_SUPPLIER = ORDERS.COD_SUPPLIER
      AND TYPE_TO_PAY.COD_TYP_PAY = FEES_TO_PAY.COD_TYP_PAY
      AND ORDERS.COD_ORDER = PURCHASE_INVOICE.COD_ORDER
      AND PURCHASE_INVOICE.COD_INVOICE = ACCOUNTS_TO_PAY.COD_INVOICE
