...
CREATE PROCEDURE SP_INS_ACCOUNTS_TO_PAY(
	        ...
	        IF NOT EXISTS(SELECT COD_INVOICE FROM ACCOUNTS_TO_PAY WHERE COD_INVOICE = _COD_INVOICE) THEN
              ROLLBACK;
			  SIGNAL SQLSTATE '45000'
...
CREATE PROCEDURE SP_INS_PAY_FORM(
  ...
  SET @USUARIOS = (SELECT COUNT(COD_USER) FROM PAY_FORM WHERE COD_USER = _COD_USER AND MONTHNAME(DAT_PAYMENT) = MONTHNAME(_DAT_PAYMENT) AND YEAR(DAT_PAYMENT)  = YEAR(_DAT_PAYMENT));
           ...
           IF NOT EXISTS(SELECT COD_USER FROM PAY_FORM WHERE COD_USER = _COD_USER) THEN
             ROLLBACK;
		     SIGNAL SQLSTATE '45000'
...
CREATE PROCEDURE SP_INS_SALES_RETURNS(
			  ...
			  IF NOT EXISTS(SELECT COD_PRODUCT FROM SALES_RETURNS WHERE COD_PRODUCT = _COD_PRODUCT) THEN
                   ROLLBACK;
			       SIGNAL SQLSTATE '45000'
